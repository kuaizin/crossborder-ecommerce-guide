...
const scenarios = [
  {
    platform: 'Amazon 卖家后台',
    needNetwork: false,
    desc: 'sellercentral.amazon.com 在国内可直接访问',
    note: '无需特殊网络',
  },
  {
    platform: 'Shopee 卖家中心',
    needNetwork: false,
    desc: 'seller.shopee.cn 为国内域名，直接访问',
    note: '无需特殊网络',
  },
  {
    platform: '速卖通卖家后台',
  ...
  },
  {
    ...
    needNetwork: true,
    desc: '部分后台功能和创作者工具需要稳定国际网络',
    note: '建议配置',
  },
  {
    platform: 'Google Trends',
    needNetwork: true,
    desc: '谷歌趋势是选品必备工具，需要国际网络',
    note: '强烈建议配置',
  },
  {
    platform: 'Facebook 广告管理',
    needNetwork: true,
    desc: '投放 Facebook/Instagram 广告必须访问',
    note: '必须配置',
  },
  {
    platform: '竞品调研（国外网站）',
    needNetwork: true,
    desc: '查看国外竞品 Listing、社交媒体趋势',
    note: '建议配置',
  },
  {
    platform: 'Shopify 后台',
    needNetwork: false,
    desc: 'admin.shopify.com 在国内通常可访问',
    note: '一般无需特殊网络',
  },
];
...
const tips = [
  {
    title: '先确认是否真的需要',
    desc: '很多平台（Shopee、速卖通）的卖家后台在国内完全可以正常访问。不要花冤枉钱。',
  },
  {
    title: '不要买太便宜的',
    desc: '几块钱一个月的服务基本不靠谱，经常断线或跑路。建议至少选择 30 元/月以上的服务。',
  },
  {
    title: '注意 IP 纯净度',
    desc: 'Amazon、TikTok 等平台会检测 IP 地址。如果 IP 被大量人共用，可能导致账号风控。尽量选择独立 IP 或专线。',
  },
  {
    title: '一个 IP 只登一个账号',
    desc: '绝对不要用一个网络环境登录多个店铺账号！这是封号的最大原因之一。每个店铺配独立的网络环境。',
  },
  {
    title: '备用方案很重要',
    desc: '至少准备两个不同的网络方案，一个主用一个备用。关键时刻不能掉链子。',
  },
  {
    title: '不要在公司网络登录店铺',
    desc: '公司内网可能有很多人共用同一个出口 IP，容易导致账号关联。店铺操作请使用独立的网络环境。',
  },
];
      ...
      <header className="gradient-primary pt-20 pb-16">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          <p className="text-lg text-white/80 max-w-2xl leading-relaxed">
            做跨境电商，哪些操作需要国际网络？如何合法、稳定、安全地配置网络环境？
            这篇指南帮你一次搞清楚。
          </p>
        </div>
      </header>
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-16">
        ...
        <div className="bg-amber-50 border border-amber-200 rounded-xl p-5 mb-12 flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-amber-800/80 leading-relaxed">
              本文仅讨论跨境电商业务所需的正常国际网络访问需求。
              请遵守当地法律法规，使用合法合规的网络服务。
              一个店铺必须使用独立的网络环境，切勿多账号共用同一 IP。
            </p>
          </div>
        </div>
        ...
        <section className="mb-16">
          ...
          <p className="text-muted-foreground mb-6">
            不是所有跨境电商操作都需要特殊网络。先搞清楚哪些场景需要，避免花冤枉钱。
          </p>
            ...
            {scenarios.map((s, idx) => (
              <div 
                ...
                className={`flex items-start gap-3 p-4 rounded-lg border ${
                  s.needNetwork ? 'bg-amber-50/50 border-amber-200' : 'bg-green-50/50 border-green-200'
                }`}
              >
                <div className={`flex h-6 w-6 shrink-0 items-center justify-center rounded-full ${
                  s.needNetwork ? 'bg-amber-100 text-amber-600' : 'bg-green-100 text-green-600'
                }`}>
                ...
                <div>
                  <div className="flex items-center gap-2">
                    ...
                    <Badge 
                      variant="outline" 
                      className={`text-xs ${s.needNetwork ? 'border-amber-300 text-amber-700' : 'border-green-300 text-green-700'}`}
                    >
                      {s.note}
                    </Badge>
                  </div>
              ...
              <div className="grid sm:grid-cols-3 gap-6">
                <div className="bg-white/10 backdrop-blur-sm rounded-xl p-5">
                  ...
                  <p className="text-white/80 text-sm leading-relaxed">
                    预算有限，不想折腾技术 → 选择 <strong className="text-white">商业代理服务</strong>，
                    即开即用，月费 30-50 元足够。先用免费/低价套餐测试稳定性。
                  </p>
                </div>
                <div className="bg-white/10 backdrop-blur-sm rounded-xl p-5">
                  <div className="text-accent font-bold text-sm mb-2">有一定经验</div>
                  <p className="text-white/80 text-sm leading-relaxed">
                    懂一点技术，想省钱又可控 → 选择 <strong className="text-white">云服务器自建</strong>，
                    月费 50-100 元，自己搭建，IP 独享更安心。
                  </p>
                </div>
                <div className="bg-white/10 backdrop-blur-sm rounded-xl p-5">
                  <div className="text-accent font-bold text-sm mb-2">团队/公司</div>
                  <p className="text-white/80 text-sm leading-relaxed">
                    多人使用，追求稳定合规 → 选择 <strong className="text-white">企业级国际专线</strong>，
                    月费 500+，但稳定、合法、可开票，适合长期经营。
                  </p>
                </div>
              </div>
            </CardContent>
