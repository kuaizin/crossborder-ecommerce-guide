...
const scenarios = [
  { platform: 'Amazon 卖家注册', needPhone: true, desc: '注册时需要接收短信验证码，部分站点支持中国手机号', note: '部分站点可用中国号' },
  { platform: 'Shopee 入驻', needPhone: false, desc: '支持中国大陆手机号直接注册', note: '无需国外手机号' },
  { platform: '速卖通（AliExpress）', needPhone: false, desc: '阿里巴巴旗下，完全支持中国手机号', note: '无需国外手机号' },
  { platform: 'TikTok Shop', needPhone: true, desc: '美区/英区店铺注册需要对应国家手机号验证', note: '必须国外手机号' },
  { platform: 'Google 账号 / Gmail', needPhone: true, desc: '注册 Google 账号用于 Google Trends、YouTube 等', note: '建议国外手机号' },
  { platform: 'Facebook / Meta 广告账户', needPhone: true, desc: '广告账户验证、双重认证需要稳定手机号', note: '建议国外手机号' },
  { platform: 'PayPal 国际版', needPhone: true, desc: '部分功能验证需要国外手机号', note: '视地区而定' },
  { platform: 'Shopify / Stripe', needPhone: true, desc: '独立站收款工具，部分需要美国/欧洲手机号', note: '视具体服务而定' },
];

const solutions = [
  {
    id: 'virtual', title: '虚拟手机号接码平台', icon: MessageSquare, color: 'bg-emerald-500', borderColor: 'border-l-emerald-500',
    description: '最快捷、最便宜的方式。购买一次性的虚拟手机号用于接收短信验证码，用完即弃或短期保留。适合注册验证场景。',
    pros: ['价格低廉', '即买即用', '支持多国家', '无需实名'],
    cons: ['号码不固定', '部分平台风控严格', '不能接电话', '有回收风险'],
    cost: '1-20 元/次', recommended: '新手、一次性注册验证',
    providers: [
  ...
  {
    id: 'esim', title: '实体 / eSIM 国外电话卡', icon: Smartphone, color: 'bg-blue-500', borderColor: 'border-l-blue-500',
    ...
    cost: '50-300 元/月', recommended: '长期经营、重要账户绑定',
    providers: [
  ...
  {
    id: 'voice', title: 'VoIP / 云电话服务', icon: Phone, color: 'bg-violet-500', borderColor: 'border-l-violet-500',
    ...
    cost: '免费 - 50 元/月', recommended: '有一定技术基础、追求性价比',
    providers: [
...
const tips = [
  { title: '重要账户用固定号码', desc: 'Amazon 卖家账户、PayPal、银行账户等核心资产，务必绑定一个你能长期控制的固定号码。不要用在接码平台买的临时号绑定重要账户！' },
  { title: '一个号码只绑一个账号', desc: '和 IP 一样，手机号也是平台检测账号关联的重要依据。同一个手机号绑定多个 Amazon 店铺 = 封号风险。' },
  { title: '接码平台选「租用」而非「一次性」', desc: 'SMS-Activate 等平台有「租用号码」选项（Rent），可以保留号码 4 小时到 4 周不等，比一次性接码更适合需要二次验证的账号。' },
  { title: 'Google Voice 需维护', desc: 'Google Voice 号码如果 6 个月内没有发送短信或拨打电话，会被 Google 回收。建议每月至少发一条短信保号。' },
  { title: '实体卡注意保号规则', desc: '不同运营商保号规则不同。例如 giffgaff 要求每 6 个月有一次消费或余额变动，Ultra Mobile 需保持月租扣费成功。' },
  { title: 'eSIM 是未来的趋势', desc: 'iPhone XS 及以上机型、大部分安卓旗舰都支持 eSIM。无需实体卡、APP 内即可购买激活，推荐 Airalo、Holafly 等服务商。' },
];
    ...
    <div className="min-h-screen bg-background">
      <header className="gradient-primary pt-20 pb-16">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          <h1 className="text-3xl sm:text-4xl font-bold text-white mb-4">跨境电商国外手机号获取指南</h1>
          <p className="text-lg text-white/80 max-w-2xl leading-relaxed">
            注册海外平台、绑定收款账户、设置双重验证都需要国外手机号。这篇指南介绍 3 种获取方式，从临时接码到长期固定号码，总有一款适合你。
          </p>
        </div>
      </header>
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-16">
        <div className="bg-amber-50 border border-amber-200 rounded-xl p-5 mb-12 flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-amber-800/80 leading-relaxed">
              核心店铺账户（Amazon、PayPal 等）务必绑定长期可控的固定号码。临时接码平台的号码不适合绑定重要资产，一旦号码被回收，账户可能永久无法找回。
            </p>
          </div>
        </div>
        ...
        <section className="mb-16">
          ...
          <p className="text-muted-foreground mb-6">不是所有平台都需要国外手机号。有些平台完全支持中国号码注册。</p>
          <div className="grid md:grid-cols-2 gap-3">
            {scenarios.map((s, idx) => (
              <div key={idx} className={`flex items-start gap-3 p-4 rounded-lg border ${s.needPhone ? 'bg-amber-50/50 border-amber-200' : 'bg-green-50/50 border-green-200'}`}>
                <div className={`flex h-6 w-6 shrink-0 items-center justify-center rounded-full ${s.needPhone ? 'bg-amber-100 text-amber-600' : 'bg-green-100 text-green-600'}`}>
                ...
                <div>
                  <div className="flex items-center gap-2">
                    ...
                    <Badge variant="outline" className={`text-xs ${s.needPhone ? 'border-amber-300 text-amber-700' : 'border-green-300 text-green-700'}`}>{s.note}</Badge>
                  </div>
            ...
            <CardContent className="p-8">
              ...
              <div className="grid sm:grid-cols-3 gap-6">
                <div className="bg-white/10 backdrop-blur-sm rounded-xl p-5">
                  ...
                  <p className="text-white/80 text-sm leading-relaxed">预算低，只需要收一次验证码 → 选择 <strong className="text-white">虚拟接码平台</strong>（SMS-Activate），每次 1-5 元，即买即用。</p>
                </div>
                <div className="bg-white/10 backdrop-blur-sm rounded-xl p-5">
                  <div className="text-accent font-bold text-sm mb-2">长期经营一个店铺</div>
                  <p className="text-white/80 text-sm leading-relaxed">有稳定店铺，需要长期固定号码 → 选择 <strong className="text-white">实体 SIM 卡 / eSIM</strong>（如 Ultra Mobile、giffgaff），月租低，号码长期可控。</p>
                </div>
                <div className="bg-white/10 backdrop-blur-sm rounded-xl p-5">
                  <div className="text-accent font-bold text-sm mb-2">会折腾、追求性价比</div>
                  <p className="text-white/80 text-sm leading-relaxed">有技术能力，愿意花时间维护 → 选择 <strong className="text-white">Google Voice / TextNow</strong>，免费或低价获得美国号码，注意定期保号。</p>
                </div>
              </div>
            </CardContent>
          </Card>
