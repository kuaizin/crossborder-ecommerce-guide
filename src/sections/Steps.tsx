...
import { BookOpen, Store, Search, FileText, Package, Truck, Wallet, CheckCircle, Lightbulb, ChevronRight, ExternalLink } from 'lucide-react';
...
const stepDetails = [
  {
    id: 1, icon: BookOpen, title: '了解基础概念', subtitle: '先搞清楚你在做什么', color: 'border-l-blue-500', bgIcon: 'bg-blue-500',
    content: {
      ...
      tips: ['跨境电商和国内电商最大的区别：要处理汇率、关税、国际物流', '新手建议从 B2C 平台起步（如 Amazon），成熟后再考虑独立站', '不需要会外语！翻译工具和中文后台已经很成熟'],
    },
  },
  {
    id: 2, icon: Store, title: '选择销售平台', subtitle: '选对平台，成功一半', color: 'border-l-indigo-500', bgIcon: 'bg-indigo-500',
    content: {
      ...
      tips: ['资金少 + 没经验 → 选 Shopee 或速卖通', '资金充足 + 想做品牌 → 选 Amazon', '有内容创作能力 → 考虑 TikTok Shop', '同一产品可以在多个平台同时销售'],
    },
  },
  {
    id: 3, icon: Search, title: '选品与市场调研', subtitle: '找到你的第一款爆品', color: 'border-l-violet-500', bgIcon: 'bg-violet-500',
    content: {
      ...
      tips: ['不要选带电/带磁/液体/粉末类产品，物流太麻烦', '避开已经有很多大卖家垄断的品类（如手机壳）', '季节性产品（如圣诞装饰）要提前 3-6 个月备货', '先从自己熟悉或感兴趣的领域入手'],
    },
  },
  {
    id: 4, icon: FileText, title: '注册开店与资质准备', subtitle: '合法合规，正式入行', color: 'border-l-purple-500', bgIcon: 'bg-purple-500',
    content: {
      ...
      tips: ['一套资料只能注册一个店铺，多店铺需要不同的营业执照和法人', '地址要真实有效，因为平台可能会寄验证码', '找靠谱的代理记账公司处理税务，别自己折腾', '如果做 Amazon，建议先学习平台规则，避免一开始就违规被封'],
    },
  },
  {
    id: 5, icon: Package, title: '商品上架与 Listing 优化', subtitle: '让产品自己卖出去', color: 'border-l-pink-500', bgIcon: 'bg-pink-500',
    content: {
      ...
      tips: ['图片是最大的转化率因素，花点钱请专业摄影师绝对值', '标题不要全大写，不要特殊符号，看起来不专业', 'Review（评价）很重要，前期可以通过 Vine 计划或站外送测获取首批评价', 'A+ 页面（图文版详情）能大幅提升转化率，有条件一定要做'],
    },
  },
  {
    id: 6, icon: Truck, title: '物流与仓储方案', subtitle: '货怎么运到买家手里', color: 'border-l-rose-500', bgIcon: 'bg-rose-500',
    content: {
      ...
      tips: ['新品测试期：自发货或小批量海运，降低风险', '产品验证后：转 FBA 或海外仓，提升配送时效和转化率', '关注物流淡旺季，旺季（11-12月）运费会涨、仓库会爆满', '打包要结实！国际运输很暴力，破损退货成本很高'],
    },
  },
  {
    id: 7, icon: Wallet, title: '收款、税务与合规', subtitle: '把钱安全地拿回来', color: 'border-l-orange-500', bgIcon: 'bg-orange-500',
    content: {
      ...
      tips: ['注册香港公司可以简化收款和税务流程，很多卖家这么做', '保留所有交易记录和发票，税务稽查时能拿出证据', '不要侵权！卖仿品、假货会被平台封店甚至法律追责', '购买商业保险（如 Amazon 要求的产品责任险），保护自己'],
    },
  },
];
    ...
    <section id="steps" className="py-20 bg-muted/30">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-16">
          ...
          <h2 className="text-3xl sm:text-4xl font-bold text-primary mb-4">每一步详细拆解</h2>
          <p className="text-lg text-muted-foreground max-w-2xl mx-auto">每个步骤都包含概念解释、操作方法和实用技巧</p>
        </div>
        <div className="space-y-8">
          {stepDetails.map((step) => {
            ...
            return (
              <Card key={step.id} id={`step-${step.id}`} className={`border-l-4 ${step.color} overflow-hidden scroll-mt-24`}>
                <CardHeader className="pb-4">
                  <div className="flex items-center gap-4">
                    <div className={`flex h-12 w-12 items-center justify-center rounded-xl ${step.bgIcon} text-white`}><Icon className="h-6 w-6" /></div>
                    <div>
                      <div className="flex items-center gap-2"><span className="text-sm font-medium text-muted-foreground">步骤 {step.id}</span></div>
                ...
                <CardContent className="pt-0">
                  <p className="text-foreground/80 mb-6 ml-16 leading-relaxed">{step.content.concept}</p>
                  <div className="ml-16 grid md:grid-cols-2 gap-4 mb-6">
                    {step.content.models.map((model: any, idx: number) => (
                      <div key={idx} className="bg-muted/50 rounded-lg p-4">
                        <div className="flex items-start justify-between gap-2">
                          <h4 className="font-semibold text-primary text-sm mb-1 flex items-center gap-2"><ChevronRight className="h-3 w-3 text-accent shrink-0" />{model.name}</h4>
                          {isPlatformStep && model.registerUrl && (
                            <a href={model.registerUrl} target="_blank" rel="noopener noreferrer" className="shrink-0">
                              <Button size="sm" variant="outline" className="h-7 text-xs gap-1 hover:bg-primary hover:text-white">注册 <ExternalLink className="h-3 w-3" /></Button>
                            </a>
                          )}
                        </div>
                  ...
                  <div className="ml-16 bg-amber-50 border border-amber-200 rounded-lg p-4">
                    <h4 className="font-semibold text-amber-800 text-sm mb-3 flex items-center gap-2"><Lightbulb className="h-4 w-4" />新手小贴士</h4>
                    <ul className="space-y-2">
                      {step.content.tips.map((tip: string, idx: number) => (
                        <li key={idx} className="flex items-start gap-2 text-sm text-amber-900/80"><CheckCircle className="h-4 w-4 text-amber-600 shrink-0 mt-0.5" />{tip}</li>
                      ))}
                    </ul>
                  </div>
                </CardContent>
