      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-16">
          ...
          <p className="text-lg text-muted-foreground max-w-2xl mx-auto">
            按照这条路径学习，从完全不懂到成功出单，每一步都有明确的目标和操作方法
          </p>
        </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-7 gap-4 lg:gap-2">
            {steps.map((step, index) => {
              ...
              return (
                ...
                >
                  <div
                    className={`relative z-10 flex h-14 w-14 items-center justify-center rounded-full text-white shadow-lg transition-all duration-300 cursor-pointer ${step.color} ${
                      activeStep === step.id ? 'scale-110 ring-4 ring-offset-2 ring-blue-200' : ''
                    }`}
                  ...
                  {index < steps.length - 1 && (
                    <ChevronRight className="lg:hidden h-5 w-5 text-muted-foreground/50 rotate-90 sm:rotate-0 mt-2 sm:mt-0" />
                  )}
                </div>
              );
            })}
